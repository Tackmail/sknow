(function(){
	...
	function loadOwnerNotes(){
		try {
			return fetch(`${API_URL}/api/notes/owner/${clientId}`)
				.then(r => r.json())
				.catch(e => { console.error('Load owner notes error:', e); return []; });
		} catch(e){
	...
	async function saveNotes(){
		// Notes are auto-synced to server now
		renderAll();
	}

	async function addNoteObj(note){
		try {
			const response = await fetch(`${API_URL}/api/notes`, {
				method: 'POST',
			...
			alert('ไม่สามารถบันทึกโน้ตได้: ' + e.message);
	...
	async function removeNote(code){
		try {
			...
			alert('ไม่สามารถลบโน้ตได้: ' + e.message);
		}
	}

	// เปลี่ยน addReply ให้รองรับ silent (ไม่แสดง toast เมื่อเป็นการตอบจากผู้ใช้คนเดียวกัน)
	async function addReply(code, msg, silent = false){
		try {
			const response = await fetch(`${API_URL}/api/notes/${code}/replies`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
	...
	function renderAll(){
		...
		if(adminMode){
			// admin mode: show list of all notes from server
			field.style.overflow = 'auto';
			field.style.display = 'block';
			fetch(`${API_URL}/api/notes/admin/all`)
				.then(r => r.json())
				.then(allNotes => {
					allNotes.forEach((n)=>{
				...
				.catch(e => console.error('Failed to load admin notes:', e));
		}else{
	...
	replyBtn.addEventListener('click', async ()=>{
		...
		if(!msg) return;
		await addReply(currentOpen.code, msg, true);
		replyInput.value = '';
		renderReplies();
	});
	...
	openByCode.addEventListener('click', ()=> {
		...
		fetch(`${API_URL}/api/notes/${c}`)
			...
			.catch(e => alert('ไม่พบโน้ตที่มีรหัสดังกล่าว'));
	});
